import{l as e}from"./index-bPHOlBSv.js";import{G as t}from"./GuidePage-ivVtlcda.js";/* empty css                 */import"./SEOHead-BOPenNwz.js";import"./index-DEmZ24Wf.js";import"./index-DH8vsOuu.js";const a={id:"glossary",icon:"fa-book-bookmark",title:"용어 해설",titleEn:"Glossary",sections:[{title:"생성형 AI 기본 용어",titleEn:"Generative AI Basics",content:`교육에서 자주 등장하는 핵심 용어를 모았습니다. 모르는 말이 나오면 여기에서 찾아보세요.
...
> Analysis starts with clean data — don't skimp on tidying.`}]},o={id:"chatbot",icon:"fa-comments",title:"챗봇 활용법",titleEn:"Chatbot Q&A",sections:[{title:"챗봇형 질의응답 이해하기",titleEn:"Understanding Chatbot Q&A",content:`챗봇형 질의응답은 **한 번 묻고 끝내는 검색이 아니라, 대화를 이어가며 원하는 결과에 다가가는 방식**입니다. 이 차이를 모른 채 한 번 물어보고 "생각보다 별로네"라고 결론짓는 것이 가장 흔한 실패입니다.
...
That is all there is to it. The rest is repetition.`}]},n={id:"file-doc",icon:"fa-folder-tree",title:"파일 요약 · 문서 정리",titleEn:"Summarize Files, Organize Documents",sections:[{title:"세 도구 중 무엇을 쓸까",titleEn:"Which of the Three to Use",content:`파일을 요약하고 문서를 정리하는 일은 **ChatGPT · Gemini · Claude 셋 다 할 수 있습니다.** 다만 잘하는 결이 조금씩 다릅니다.
...
**✅ Check** — save it to a shared document so **anyone produces the same format.**`}]},i={id:"image-gen",icon:"fa-wand-magic-sparkles",title:"이미지 만들기",titleEn:"Making Images",sections:[{title:"내 사진을 살리고, 홍보 이미지 만들기",titleEn:"Restore Your Photos, Make Your Own Graphics",content:`**사진 복원·보정 → 배경 제거 → 프로필/카드/포스터 완성**
...
> **꼭 기억해요** — 여기서는 사진·이미지에만 집중해요. PPT나 영상은 다루지 않아요.`,contentEn:`**Restore and retouch → remove the background → finish a profile photo, card or poster**
...
> **Remember** — this is about photos and images only. Slides and video are not covered here.`},{title:"따라하기 ① 옛날 사진 복원 · 흐린 사진 선명하게",titleEn:"Walkthrough 1 — Restore Old or Blurry Photos",content:`> **목표** — 스캔했거나 오래돼 색이 바랜 사진, 초점이 흐린 사진을 또렷하게 되살린다.
...
| 무료 사진 복원/업스케일 웹 도구 (예시 — 강사 재량으로 대체) |`,contentEn:`> **Goal** — Bring back a scanned, faded or out-of-focus photo.
...
| A free photo restoration/upscaling web tool (example — the instructor may substitute) |`},{title:"따라하기 ② 프로필 사진 보정 · 배경 지우기·바꾸기",titleEn:"Walkthrough 2 — Retouch a Profile Photo and Change the Background",content:`> **목표** — 증명사진처럼 쓸 수 있는 프로필 사진 한 장을 완성한다.
...
|------|
| 배경 제거 전용 무료 웹 도구 (예시) |
| 대화형 AI의 이미지 편집 기능 (예시) |`,contentEn:`> **Goal** — Finish one profile photo you could use like an ID photo.
...
| The image editing feature of a conversational AI (example) |`},{title:"따라하기 ③ 원하는 이미지를 얻는 설명법",titleEn:"Walkthrough 3 — How to Describe What You Want",content:`> **목표** — "대상·장면·분위기·비율" 네 가지를 넣어 원하는 이미지를 정확히 얻는다.
...
Size — square (1:1).
\`\`\``},{title:"따라하기 ④ 명절카드·모임 포스터·가게 홍보물 중 하나 완성",titleEn:"Walkthrough 4 — Finish a Card, Poster or Shop Graphic",content:`> **목표** — 배운 것을 모두 합쳐 실제로 쓸 결과물 1건을 끝까지 완성한다.
...
| 대화형 AI로 문구 초안 작성 (예시) |`,contentEn:`> **Goal** — Combine everything into one finished piece you will actually use.
...
| A conversational AI for drafting the wording (example) |`},{title:"따라하기 ⑤ 내 사진으로 이모티콘 만들기",titleEn:"Walkthrough 5 — Turn Your Photo into Emoji",content:`**소요 20분 · 준비물: 얼굴이 정면으로 크게 나온 내 사진 1장**
...
> 내 얼굴이라도 **원본 사진은 지우지 말고 보관**하세요. 다시 만들 때 필요합니다.`,contentEn:`**20 minutes · You need one photo of your face, front-on and large in frame**
...
> Keep the original photo — you will need it to regenerate.`},{title:"따라하기 ⑥ 내 사진을 지브리풍 이미지로 만들기",titleEn:"Walkthrough 6 — Turn Your Photo into a Ghibli-Style Image",content:`**소요 20분 · 준비물: 인물이 크게 나온 사진 1장**
...
> 시정 홍보물처럼 **대외에 나가는 자료**라면 부서의 AI 이용 지침을 먼저 확인하세요. 지침이 이 문서보다 우선합니다.`,contentEn:`**20 minutes · You need one photo with the person large in frame**
...
> For anything public-facing, check your organization's AI-use policy first — it takes precedence over this page.`},{title:"따라하기 ⑦ 메타 프롬프트 — 프롬프트를 AI가 대신 써 주게 하기",titleEn:"Walkthrough 7 — Meta Prompts: Let the AI Write Your Prompt",content:`**소요 15분 · 앞의 따라하기 ①~⑥을 한 번 해 본 뒤에 하면 가장 좋습니다**
...
> 메타 프롬프트를 부서 공유 문서에 저장해 두세요. **프롬프트를 잘 쓰는 사람이 없어도, 부서 전체가 같은 품질의 이미지를 만들 수 있습니다.**`,contentEn:`**15 minutes · Best done after you have tried Walkthroughs 1–6 once**
...
> Save them in a shared document. **Even with nobody on the team who is good at prompts, the whole team can produce consistent images.**`},{title:"막힐 때 이렇게 해보세요",titleEn:"When You Get Stuck",content:`### 원하는 그림이 안 나와요
...
> 여기서 익힌 이미지 감각은 가짜 이미지를 판별하는 눈으로 그대로 이어집니다.`,contentEn:`### I can't get the picture I want
...
> The eye you develop here carries straight over into spotting fake images.`}]},s={id:"ai-models",icon:"fa-microchip",title:"AI 모델 고르기",titleEn:"Choosing a Model",sections:[{title:"한눈에 고르는 법",titleEn:"Quick Guide",content:`> AI 플랫폼에서는 여러 회사의 AI 모델을 골라 쓸 수 있습니다. **잘 모르겠으면 \`Auto(자동)\`** — 작업에 맞는 모델을 알아서 골라줍니다. 표시 뜻: **(고비용)** = 요금이 많이 드니 꼭 필요할 때만 · **(New)** = 최신 모델.
...
| Grok Code Fast | coding | fast code tasks | |`}]},r={id:"tips",icon:"fa-lightbulb",title:"실전 팁",titleEn:"Practical Tips",sections:[{title:"좋은 답을 얻는 프롬프트 팁",titleEn:"Tips for Better Prompts",content:`같은 AI라도 어떻게 묻느냐에 따라 결과가 크게 달라집니다.
...
- Always minimize sensitive data when using AI for work.`}]},l={id:"resources",icon:"fa-graduation-cap",title:"함께 공부할 자료",titleEn:"Study Together",sections:[{title:"과정별 추천 학습 순서",titleEn:"Recommended Learning Path",content:`교육이 끝난 뒤에도 스스로 이어서 공부할 수 있도록 순서를 제안합니다.
...
> Stuck? Come back to Appendix → Glossary and Practical Tips.`}]};function g(){return e.jsx(t,{seoTitle:"부록",seoTitleEn:"Appendix",seoDescription:"용어 해설, 챗봇 활용법, 파일 요약·문서 정리, 이미지 만들기, AI 모델 고르기, 실전 팁, 함께 공부할 자료 — 교육과 더불어 공부할 부록 자료",path:"/appendix",dataFiles:[a,o,n,i,s,r,l]})}export{g as default};
